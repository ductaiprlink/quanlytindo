                    ...
                    <form class="form-horizontal" action="{{ route('users.update', ['id' => $user->id]) }}" method="post">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Tên quản trị viên" value="{{ $user->name }}">
                                    @if ($errors->has('name'))
                            ...
                            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" id="email" name="email" placeholder="Email quản trị viên" value="{{ $user->email }}">
                                    @if ($errors->has('email'))
                                        <span class="help-block">
                                ...
                                <div class="col-sm-9">
                                    <select name="roles" id="roles" class="form-control">
                                        @foreach($roles as $role)
                                            <option value="{{ $role->id }}" @if ($role->id == $roleUser->role_id) selected @endif>{{ $role->display_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            {{-- xác thực 2 bước --}}
                            <div class="form-group">
                                <label for="roles" class="col-sm-3 control-label">Google 2FA</label>

                                <div class="col-sm-9">
                                    <a href="{{ route('2fa')  }}" class="btn btn-warning btn-flat">Enable (or Disable)</a>
                                </div>
                            </div>
